"""Rendering for :class:`~stoner.types.VoiceReport` in three formats.
...
printing to a terminal), ``"markdown"``, and ``"json"``. Mirrors
:mod:`stoner.slop.report` exactly, with voice verdict bands.
"""
...
VERDICT_BANDS: list[tuple[float, str]] = [
    ...
    (30.0, "drifting"),
    (55.0, "off voice"),
    (float("inf"), "broke voice"),
]


def verdict(score: float) -> str:
    """Map a 0-100 drift score to a human verdict band."""
...
def render(report: VoiceReport, fmt: str = "rich") -> str:
    """Render a VoiceReport as ``"rich"``, ``"markdown"``, or ``"json"`` text."""
    if fmt == "json":
...
def _render_markdown(report: VoiceReport) -> str:
    v = verdict(report.score)
    lines = [
        f"# Voice report: {report.path or '(unnamed)'}",
        "",
        f"**Drift score:** {report.score:.1f} / 100 -- **{v}**",
        "",
        "## Subscores",
        "",
        "| bucket | subscore |",
    ...
    for f in findings:
        ...
        lines.append(f"- **[{f.severity.value}]** ({f.category}) {loc}: {f.issue}")

    lines += ["", "## Stats", "", "```json", json.dumps(report.stats, indent=2, default=str), "```"]
    return "\n".join(lines)


def _render_rich(report: VoiceReport) -> str:
    import io
    ...
    v = verdict(report.score)
    console.print(f"[bold]Voice report[/bold]: {report.path or '(unnamed)'}")
    console.print(f"Drift score: [bold]{report.score:.1f}[/bold] / 100 -- verdict: [bold]{v}[/bold]")

    sub_table = Table(title="Subscores")
    sub_table.add_column("Bucket")
